...
void gui::ProgressBar::update(const int current_value, const int max_value)
{
	...
	if (percent > 1.f)
        percent = 1.f;
    // Agar bar tidak pernah di bawah 0% (error aneh lainnya)
    if (percent < 0.f)
        percent = 0.f;
		
    this->inner.setSize(
        sf::Vector2f(
            // mending pake maxWidth (Lebar Pixel Layar) dikali persentase
            // daripada static_cast<float>(std::floor(max_value * percent)), 
